...
- What happens when the shopper unchecks a subcategory (including a nested one that itself has children)? The filter moves UP to that subcategory's immediate parent — the top-level category MUST remain checked. Only unchecking a top-level node (one with no parent in the facet tree) clears the subcategory filter entirely.
...
- **FR-002**: The product listing page MUST allow the shopper to select and deselect subcategory filters, updating the visible product list accordingly. Deselecting a nested subcategory MUST move the filter UP to its immediate parent (keeping the top-level category selected) rather than clearing the entire selection; only deselecting a top-level node clears the subcategory filter entirely.
